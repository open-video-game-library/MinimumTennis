...
public class FirstPlayerController : MonoBehaviour
{
    ...
    void OnTriggerStay(Collider other)
    {
        if (other.gameObject.CompareTag("Ball") 
            && !isHit && manager.active && !manager.isTOS && !manager.isSERVE && manager.who != "Player1")
        {
        ...
        else if (other.gameObject.CompareTag("Ball") && !isHit && manager.active && manager.isTOS)
        {
            ...
            if (Input.GetKeyDown(KeyCode.Space) || controllerManager.PressEastButton(0))
            {
    ...
    private void ControlPlayer(GameObject _ball, float _vz)
    {
        ...
        if (!manager.isOUT) manager.who = "Player1";
        manager.ballCount = 0;
    }
    ...
    private void Serve(float _power)
    {
        ...
        if (controllerManager.leftStick[0].x > 0.10f)
        {
            ballController.vz = -0.50f * manager.whereServe + 0.20f * controllerManager.leftStick[0].x;
        }
        else if (controllerManager.leftStick[0].x < -0.10f)
        {
            ballController.vz = -0.5f * manager.whereServe + 0.20f * controllerManager.leftStick[0].x;
        }
    ...
    private void ExtendFrameEast()
    {
        ...
        {
            normalFrames = 30;
            normalShot = Input.GetKeyDown(KeyCode.L) || controllerManager.PressEastButton(0);
        }
    }
    ...
    private void ExtendFrameSouth()
    {
        ...
        {
            lobFrames = 30;
            lobShot = Input.GetKeyDown(KeyCode.K) || controllerManager.PressSouthButton(0);
        }
    }
    ...
    private void ExtendFrameWest()
    {
        ...
        {
            dropFrames = 30;
            dropShot = Input.GetKeyDown(KeyCode.J) || controllerManager.PressWestButton(0);
        }
    }
    ...
    private void ExtendFrameNorth()
    {
        ...
        {
            fastFrames = 30;
            fastShot = Input.GetKeyDown(KeyCode.I) || controllerManager.PressNorthButton(0);
        }
    }
}
