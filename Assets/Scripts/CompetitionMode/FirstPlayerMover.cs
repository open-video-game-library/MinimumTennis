...
public class FirstPlayerMover : MonoBehaviour, ICharacterMover
{
    ...
    void Update()
    {
        ...
        {
            if (gamepad.isConnected[0]) { MovePlayerGamepad(movementRangeBegin, movementRangeEnd); }
        }
    }
    ...
    {
        if (gamepad.leftStickValue[0].x < -0.10f)
        {
            // move left
            if (speedZ - playerAcceleration > -playerMaximumSpeed * gamepad.leftStickAbsValue[0].x) { speedZ -= playerAcceleration; }
            else { speedZ = -playerMaximumSpeed * gamepad.leftStickAbsValue[0].x; }
        }
        else if (gamepad.leftStickValue[0].x > 0.10f)
        {
            // move right
            if (speedZ + playerAcceleration < playerMaximumSpeed * gamepad.leftStickAbsValue[0].x) { speedZ += playerAcceleration; }
            else { speedZ = playerMaximumSpeed * gamepad.leftStickAbsValue[0].x; }
        ...
        if (gamepad.leftStickValue[0].y > 0.10f)
        {
            // move forward
            if (speedX - playerAcceleration > -playerMaximumSpeed * gamepad.leftStickAbsValue[0].y) { speedX -= playerAcceleration; }
            else { speedX = -playerMaximumSpeed * gamepad.leftStickAbsValue[0].y; }
        }
        else if (gamepad.leftStickValue[0].y < -0.10f)
        {
            // move back
            if (speedX + playerAcceleration < playerMaximumSpeed * gamepad.leftStickAbsValue[0].y / 2.0f) { speedX += playerAcceleration; }
            else { speedX = playerMaximumSpeed * gamepad.leftStickAbsValue[0].y / 2.0f; }
        }
        else { speedX *= 0.90f; }
    ...
    private void RotateCharacter(float currentSpeed, float maxSpeed)
    {
        ...
        if (gamepad.leftStickValue[0].x < -0.10f || gamepad.leftStickValue[0].x > 0.10f)
        {
