...
public class SecondPlayerShotController : MonoBehaviour
{
    ...
    void OnTriggerStay(Collider other)
    {
        ...
        if (GameManager.inPlay && !GameManager.isToss && GameManager.isServeIn && GameManager.lastShooter != name)
        {
            ...
            if (GameManager.courtAreaBegin.x / 2.0f < transform.position.x
                && ballHight > transform.localScale.y * 3.5f)
            {
                if (gamepad.isAnyButtonPressed[1]) { Smash(); }
            }
            else if (GameManager.courtAreaBegin.x / 2.0f < transform.position.x
                && ballHight > transform.localScale.y * 0.70f)
            {
                if (gamepad.isAnyButtonPressed[1]) { Volley(); }
            }
            else
            {
                if (gamepad.isPressedEast[1]) { NormalShot(); }
                else if (gamepad.isPressedSouth[1]) { LobShot(); }
                else if (gamepad.isPressedNorth[1]) { FastShot(); }
                else if (gamepad.isPressedWest[1]) { DropShot(); }
            }
        }
        else if (GameManager.inPlay && GameManager.isToss)
        {
            if (gamepad.isPressedEast[1]) { Serve(); }
        }
    }
    ...
    private void Serve()
    {
        ...
        {
            float leftStickX = gamepad.leftStickValue[1].x;
